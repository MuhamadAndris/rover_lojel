...
export default function IncomingStockPage() {
  const [data, setData] = useState<IncomingRecord[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/stock-incoming?${params}`);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button onClick={() => setFormOpen(true)} className="btn-primary ml-auto"><Plus className="w-4 h-4" /> Catat Barang Datang</button>
      </div>
        ...
        <div className="overflow-x-auto scrollbar-thin">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-ink-50">
              ...
              : data.length === 0 ? <tr><td colSpan={8}><EmptyState title="Belum ada data barang datang" description="Catat penerimaan barang dari supplier." action={<button onClick={() => setFormOpen(true)} className="btn-primary text-sm"><Plus className="w-4 h-4" /> Catat Barang Datang</button>} /></td></tr>
              : data.map((r) => (
                <tr key={r._id} className="hover:bg-ink-50/50">
                  <td className="px-4 py-3 font-mono text-xs text-ink-700">{r.incomingId}</td>
                  ...
                  <td className="px-4 py-3 font-semibold text-brand-700">+{r.totalQty}</td>
      ...
      {formOpen && (
        <IncomingForm onClose={() => setFormOpen(false)} onSuccess={() => { setFormOpen(false); fetchData(); }} />
      )}

      <GenericImportModal
        open={importOpen}
        title="Import Barang Datang"
        importUrl="/api/stock-incoming/import"
        templateUrl="/api/stock-incoming/template"
        templateFileLabel="Download template CSV barang datang"
...
/* ---------------- FORM ---------------- */
function IncomingForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const [items, setItems] = useState<IncomingItem[]>([
    { itemId: generateItemId(), productId: '', productDescription: '', qty: 1 },
  ]);
  const [saving, setSaving] = useState(false);
  const fileRef = useRef<HTMLInputElement>(null);

  function addItem() {
    setItems((prev) => [...prev, { itemId: generateItemId(), productId: '', productDescription: '', qty: 1 }]);
  }
  function updateItem(idx: number, field: keyof IncomingItem, value: string | number) {
  ...
  async function handlePhotoUpload(file: File) {
    ...
    try {
      ...
      const res = await fetch('/api/stock-incoming/upload', { method: 'POST', body: form });
      const json = await res.json();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error);
      toast.success('Barang datang berhasil dicatat. Stok ter-update otomatis.');
      onSuccess();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto py-8">
      ...
      <form onSubmit={handleSubmit} className="relative bg-white rounded-xl2 shadow-panel w-full max-w-2xl mx-4 p-6 space-y-4 my-auto max-h-[90vh] overflow-y-auto scrollbar-thin">
        <div className="flex items-center justify-between">
          <h2 className="font-display font-semibold text-ink-900">Catat Barang Datang</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-ink-600 mb-1">Nomor Surat Jalan</label>
          ...
          <div className="space-y-2">
            {items.map((item, idx) => (
              <div key={item.itemId} className="flex gap-2">
                ...
                <input type="number" min={1} placeholder="Qty" value={item.qty} onChange={(e) => updateItem(idx, 'qty', parseInt(e.target.value) || 1)} className="input-base text-xs w-20 text-center" required />
